...
import {
  ...
  Gem, ShoppingBag, Eye, ShieldCheck, Tag, Gift
} from 'lucide-react';
...
// ── DATA ────────────────────────────────────────────────────────────────────────
const STATS = [
  { v: '4×', l: 'Higher catalog engagement' },
  { v: '65%', l: 'More store visits booked' },
  { v: '5–6h', l: 'Sales staff hours saved' },
  { v: '24/7', l: 'VIP concierge AI' },
];

type Card = { icon: LucideIcon; t: string; d: string };

const CHALLENGES: Card[] = [
  { icon: Eye, t: 'High-value leads lost in DMs', d: 'Buyers ask about designs, gold rates, and availability on Instagram & WhatsApp but get slow replies.' },
  { icon: ShoppingBag, t: 'Manual catalog sharing', d: 'Sales teams spend hours sending individual images and PDF catalogs to prospective buyers.' },
  { icon: Gift, t: 'Gold scheme & festive recall', d: 'Chasing monthly gold scheme installments and festive re-purchases manually is time-consuming.' },
];

const CAPABILITIES: Card[] = [
  { icon: Gem, t: 'Interactive WhatsApp catalog', d: 'Showcase gold, diamond and bridal collections directly in rich interactive WhatsApp catalogs.' },
  { icon: CalendarCheck, t: 'Vip boutique appointments', d: 'Allow buyers to schedule private store visits and custom design consultations seamlessly.' },
  { icon: BellRing, t: 'Gold rate & scheme alerts', d: 'Send daily gold rate updates and monthly gold saving scheme payment reminders.' },
  { icon: Sparkles, t: 'AI sales concierge', d: 'Answers design, purity, and pricing inquiries instantly 24/7 with human sales handoff.' },
  { icon: CreditCard, t: 'Instant advance & UPI links', d: 'Share secure UPI and card payment links for custom design deposits and scheme payments.' },
  { icon: ShieldCheck, t: 'Certificate & invoice delivery', d: 'Send BIS hallmark certificates, digital invoices, and insurance documents straight to chat.' },
];

const HIGHLIGHTS = [
  {
    tag: 'Catalog & VIP Appointments',
    title: 'Showcase luxury collections and book in-store visits',
    body: 'Give buyers a personal concierge experience on WhatsApp. Send curated digital catalogs and let them book private showroom appointments in seconds.',
    points: ['Rich interactive product catalog browsing', 'Automated store appointment booking', 'Custom design inquiry & customization intake'],
  },
  {
    tag: 'Gold Schemes & Retention',
    title: 'Automate monthly installment payments & festive sales',
    body: 'Send automated WhatsApp payment nudges for gold schemes and retarget existing customers with festive and anniversary collection drops.',
    points: ['Monthly gold scheme payment reminders', 'One-tap UPI payment link checkout', 'Personalized anniversary & birthday offers'],
  },
];

const TESTIMONIALS = [
  { q: 'Catalog sharing and VIP store appointment booking on WhatsApp increased our high-ticket sales by 40%.', c: 'Orra Jewels', r: 'Luxury Jewellery' },
  { q: 'Gold saving scheme installment collection used to take endless phone calls. Now WeNext automates 80% of reminders.', c: 'Kalyan Heritage', r: 'Gold & Diamond' },
  { q: 'Custom design inquiries are handled instantly by AI and handed off to our master jeweler when ready.', c: 'Zaveri & Co.', r: 'Bridal Jewellery' },
];

const FAQS = [
  { q: 'Can we showcase our full jewellery catalog on WhatsApp?', a: 'Yes. WeNext supports WhatsApp Interactive Catalogs, allowing buyers to browse collections, filter by metal or occasion, and request quotes directly.' },
  { q: 'How does store appointment booking work?', a: 'Buyers select their preferred showroom location, date, and time slot right inside WhatsApp. The appointment is automatically synced with your store calendar.' },
  { q: 'Can we automate gold rate and scheme payment reminders?', a: 'Yes. You can send daily live gold rate updates and automated monthly scheme installment reminders with embedded payment links.' },
  { q: 'Is high-value customer transaction data secure?', a: 'Yes. WeNext uses official WhatsApp Business API infrastructure with end-to-end encryption and enterprise security standards.' },
  { q: 'How do sales representatives take over from the AI assistant?', a: 'When a customer requests custom pricing or a live design consultation, WeNext immediately routes the chat to the assigned sales advisor with full context.' },
];

const BENEFITS: { icon: LucideIcon; t: string; c: string; d: string; items: string[] }[] = [
  { icon: Users, t: 'Showroom & Sales Team', c: '#0a8f5a', d: 'Drive footfall and close high-value sales faster.', items: ['Interactive catalog sharing in chat', 'VIP store visit appointment booking', 'Instant gold rate & purity inquiry responses'] },
  { icon: Stethoscope, t: 'Custom Design & Craftsmen', c: '#3f6cab', d: 'Streamline custom design approvals & updates.', items: ['Share CAD designs & 3D renders on WhatsApp', 'Instant design approval from buyers', 'Progress updates on custom orders'] },
  { icon: Megaphone, t: 'Marketing & Festive Growth', c: '#7c5bd6', d: 'Retarget buyers during weddings & festivals.', items: ['Akshaya Tritiya & Diwali campaign broadcasts', 'Automate wedding season bridal collection drops', 'Personalized anniversary & birthday re-engagement'] },
  { icon: Receipt, t: 'Scheme Accounts & Billing', c: '#c98a3a', d: 'Automate gold scheme collections & invoices.', items: ['Automated monthly scheme payment links', 'BIS hallmark digital certificate delivery', 'Instant advance deposit checkout'] },
];
    ...
    <div className="bg-[#f8f5ec] relative size-full min-h-screen flex flex-col overflow-x-clip">
      ...
      <div className="bg-[#092511] shadow-[0_0_0_100vmax_#092511] [clip-path:inset(0_-100vmax)] shrink-0 w-full">
        <div className="container mx-auto border-x border-[rgba(255,255,255,0.08)] px-4 xl:px-[75px] py-[90px] lg:py-[100px] flex flex-col items-center text-center relative overflow-hidden">
          ...
          <Eyebrow label="Jewellery Solution" light center />
          ...
          <Typography component="h1" className="font-['Geist:SemiBold'] font-semibold leading-[1.14] text-white text-[44px] md:text-[60px] tracking-[-2px] max-w-[840px] mb-[20px] relative">
            WhatsApp Catalogs,<br />VIP Bookings & <span className="text-[#25d366]">Gold Scheme Automation</span>
          </Typography>

          <Typography className="font-['Geist:Regular'] text-[#a0b8a8] text-[18px] md:text-[20px] max-w-[660px] leading-[1.6] mb-[40px] relative">
            Turn social media inquiries into high-ticket sales — interactive catalogs, VIP showroom appointments, gold rate alerts and 24/7 AI sales assistance.
          </Typography>

          <div className="flex flex-wrap gap-[16px] items-center justify-center relative">
            <PrimaryButton label="Book a Jewellery Demo" />
      ...
      <div className="container mx-auto border-x border-b border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[36px] flex flex-col items-center gap-[20px]">
          <Typography className="font-['Courier_Prime'] text-[#8a938e] text-[12.5px] uppercase tracking-[0.18em]">Trusted by premium jewellers</Typography>
          <div className="flex flex-wrap items-center justify-center gap-x-[44px] gap-y-[18px]">
            {['Orra Jewels', 'Kalyan Heritage', 'Zaveri & Co.', 'Shree Diamonds', 'Royal Gold'].map((n) => (
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            What’s quietly losing you high-ticket buyers
          </Typography>
          <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] max-w-[560px] leading-[1.55] mt-[14px]">
            Three everyday sales bottlenecks cost jewellery brands footfall and revenue — WeNext fixes each one.
          </Typography>
        </div>
      </div>
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            Built specifically for jewellery brands
          </Typography>
        </div>
      </div>
      <div className="container mx-auto">
        ...
        <div className="px-4 xl:px-[56px] py-[64px]">
          <div className="max-w-[600px] mb-[40px]">
            ...
            <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[42px] text-[#0c221f] tracking-[-1.4px] leading-[1.14]">
              How your whole business benefits
            </Typography>
            <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] leading-[1.55] mt-[14px]">
              One shared WhatsApp workspace — sales, design, marketing and billing working together in sync.
            </Typography>
          </div>
              ...
              <div className="content-stretch flex flex-col gap-[30px] items-start relative shrink-0 w-full lg:w-[380px]">
                <div className="content-stretch flex flex-col gap-[15px] items-start relative shrink-0">
                  ...
                  <div className="font-['Geist:SemiBold'] font-semibold relative shrink-0 text-[#0c221f] text-[36px] lg:text-[42px] tracking-[-1px]">
                    <Typography className="leading-[1.3] text-[#0c221f] text-[36px] lg:text-[42px] font-['Geist:SemiBold'] font-semibold">Questions jewellers ask</Typography>
                  </div>
                </div>
              </div>
