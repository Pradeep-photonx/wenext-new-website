...
import {
  ...
  ShieldCheck, Shield, FileCheck, AlertCircle, PhoneCall, RefreshCw
} from 'lucide-react';
...
// ── DATA ────────────────────────────────────────────────────────────────────────
const STATS = [
  { v: '35%', l: 'Higher policy renewal rate' },
  { v: '8×', l: 'Faster quote generation' },
  { v: '5–6h', l: 'Claim desk hours saved' },
  { v: '24/7', l: 'Always-on insurance AI' },
];

type Card = { icon: LucideIcon; t: string; d: string };

const CHALLENGES: Card[] = [
  { icon: RefreshCw, t: 'Lapsed policy renewals', d: 'Policyholders miss renewal deadlines due to unread email reminders and unanswered calls.' },
  { icon: MessageCircle, t: 'High claim status inquiries', d: 'Policyholders constantly call to check claim progress, approval stage, and payout timelines.' },
  { icon: ClipboardList, t: 'Manual quote & KYC intake', d: 'Insurance agents spend hours manually collecting documents and calculating policy quotes.' },
];

const CAPABILITIES: Card[] = [
  { icon: BellRing, t: 'Automated renewal nudges', d: 'Send policy renewal reminders with instant online payment links on WhatsApp before expiry.' },
  { icon: FileCheck, t: 'Instant quote calculator', d: 'Calculate motor, health, or life insurance quotes directly inside WhatsApp in seconds.' },
  { icon: ShieldCheck, t: 'Claim status & doc upload', d: 'Allow policyholders to upload claim documents and track claim status live on WhatsApp.' },
  { icon: Sparkles, t: 'AI insurance assistant', d: 'Answers policy coverage, deductible, and terms & conditions questions 24/7.' },
  { icon: CreditCard, t: 'Premium payment checkout', d: 'Share secure premium payment links and generate instant tax-exempt receipts.' },
  { icon: FileText, t: 'Digital policy PDF delivery', d: 'Deliver official policy documents, health cards, and renewal receipts straight to chat.' },
];

const HIGHLIGHTS = [
  {
    tag: 'Policy Renewals & Instant Quotes',
    title: 'Automate policy renewals and generate quotes in seconds',
    body: 'Prevent lapsed policies with automated WhatsApp renewal reminders. Let prospective policyholders calculate quotes and buy coverage inside WhatsApp.',
    points: ['Automated policy expiry & renewal nudges', 'Instant quote calculation for motor, health & life', 'Embedded premium payment links with tax receipts'],
  },
  {
    tag: 'Claims & Document Intake',
    title: 'Streamline claim registration and document collection',
    body: 'Policyholders submit photos, bills, and claim forms on WhatsApp. Send automated claim progress updates from registration to final settlement.',
    points: ['WhatsApp document & bill photo upload', 'Real-time claim status & approval alerts', 'Instant digital policy document PDF delivery'],
  },
];

const TESTIMONIALS = [
  { q: 'Policy renewal rates increased by 38% after implementing automated WhatsApp renewal reminders with instant payment links.', c: 'SecureLife Insurance', r: 'General & Health' },
  { q: 'Claim tracking calls dropped by 70%. Customers love checking their claim status on WhatsApp in seconds.', c: 'Apex Shield', r: 'Motor & Health Broker' },
  { q: 'Our agents quote 3x more policies per day because WeNext’s AI collects vehicle details and KYC documents first.', c: 'Vanguard Risk Advisors', r: 'Insurance Agency' },
];

const FAQS = [
  { q: 'How does automated policy renewal work on WhatsApp?', a: 'WeNext triggers automated renewal alerts 30, 15, and 3 days before policy expiry, complete with renewal quote details and direct payment links.' },
  { q: 'Can policyholders submit claim documents via WhatsApp?', a: 'Yes. Policyholders can upload photos of bills, medical reports, or vehicle damage directly in WhatsApp, which syncs with your claims CRM.' },
  { q: 'Does it support instant premium payment processing?', a: 'Yes. WeNext generates secure payment links via Razorpay, PayU, or UPI gateways for instant policy issuance.' },
  { q: 'Is it compliant with insurance regulatory and data privacy standards?', a: 'Yes. WeNext operates on official WhatsApp Business API infrastructure with enterprise-grade encryption and strict privacy protocols.' },
  { q: 'Can insurance agents take over when complex advice is required?', a: 'Yes. When a prospect requires custom underwriting or advice, the chat transfers immediately to an assigned insurance advisor.' },
];

const BENEFITS: { icon: LucideIcon; t: string; c: string; d: string; items: string[] }[] = [
  { icon: Users, t: 'Agents & Sales Advisory', c: '#0a8f5a', d: 'Close more policies with qualified leads.', items: ['Instant quote calculation on WhatsApp', 'Automated KYC & document intake bot', 'Lead transfer to licensed insurance agents'] },
  { icon: Stethoscope, t: 'Claims & Underwriting Desk', c: '#3f6cab', d: 'Faster claim processing and document verification.', items: ['WhatsApp photo & bill upload for claims', 'Live claim progress notifications', 'Digital policy PDF dispatch'] },
  { icon: Megaphone, t: 'Renewal & Marketing Desk', c: '#7c5bd6', d: 'Boost policy renewal rates and cross-sells.', items: ['Automated 30/15/3 day renewal nudges', 'Cross-sell top-up & rider campaigns', 'Festive discount & offer broadcasts'] },
  { icon: Receipt, t: 'Billing & Premium Collection', c: '#c98a3a', d: 'Zero friction premium payment collection.', items: ['Send premium payment checkout links', 'Generate instant tax 80D receipts', 'Auto-sync payment status to core system'] },
];
    ...
    <div className="bg-[#f8f5ec] relative size-full min-h-screen flex flex-col overflow-x-clip">
      ...
      <div className="bg-[#092511] shadow-[0_0_0_100vmax_#092511] [clip-path:inset(0_-100vmax)] shrink-0 w-full">
        <div className="container mx-auto border-x border-[rgba(255,255,255,0.08)] px-4 xl:px-[75px] py-[90px] lg:py-[100px] flex flex-col items-center text-center relative overflow-hidden">
          ...
          <Eyebrow label="Insurance Solution" light center />
          ...
          <Typography component="h1" className="font-['Geist:SemiBold'] font-semibold leading-[1.14] text-white text-[44px] md:text-[60px] tracking-[-2px] max-w-[840px] mb-[20px] relative">
            WhatsApp Renewals,<br />Instant Quotes & <span className="text-[#25d366]">Claim Automation</span>
          </Typography>

          <Typography className="font-['Geist:Regular'] text-[#a0b8a8] text-[18px] md:text-[20px] max-w-[660px] leading-[1.6] mb-[40px] relative">
            Transform insurance operations on WhatsApp — automated policy renewals, instant quote generation, claim tracking and 24/7 AI policy assistant.
          </Typography>

          <div className="flex flex-wrap gap-[16px] items-center justify-center relative">
            <PrimaryButton label="Book an Insurance Demo" />
      ...
      <div className="container mx-auto border-x border-b border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[36px] flex flex-col items-center gap-[20px]">
          <Typography className="font-['Courier_Prime'] text-[#8a938e] text-[12.5px] uppercase tracking-[0.18em]">Trusted by leading insurance brokers & insurers</Typography>
          <div className="flex flex-wrap items-center justify-center gap-x-[44px] gap-y-[18px]">
            {['SecureLife Insurance', 'Apex Shield', 'Vanguard Risk Advisors', 'TrustGuard Policy', 'Metro Insurance'].map((n) => (
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            What’s quietly driving policy lapse rates
          </Typography>
          <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] max-w-[560px] leading-[1.55] mt-[14px]">
            Three major operational bottlenecks slow insurance renewal and claims — WeNext solves each one.
          </Typography>
        </div>
      </div>
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            Built specifically for insurance agencies & brokers
          </Typography>
        </div>
      </div>
      <div className="container mx-auto">
        ...
        <div className="px-4 xl:px-[56px] py-[64px]">
          <div className="max-w-[600px] mb-[40px]">
            ...
            <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[42px] text-[#0c221f] tracking-[-1.4px] leading-[1.14]">
              How your whole agency benefits
            </Typography>
            <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] leading-[1.55] mt-[14px]">
              One shared WhatsApp workspace — sales, claims, renewals and billing working together in sync.
            </Typography>
          </div>
              ...
              <div className="content-stretch flex flex-col gap-[30px] items-start relative shrink-0 w-full lg:w-[380px]">
                <div className="content-stretch flex flex-col gap-[15px] items-start relative shrink-0">
                  ...
                  <div className="font-['Geist:SemiBold'] font-semibold relative shrink-0 text-[#0c221f] text-[36px] lg:text-[42px] tracking-[-1px]">
                    <Typography className="leading-[1.3] text-[#0c221f] text-[36px] lg:text-[42px] font-['Geist:SemiBold'] font-semibold">Questions insurance teams ask</Typography>
                  </div>
                </div>
              </div>
