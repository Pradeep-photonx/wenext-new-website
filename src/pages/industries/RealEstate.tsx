...
import {
  ...
  Building, MapPin, Key, Home, Compass, Layers
} from 'lucide-react';
...
// ── DATA ────────────────────────────────────────────────────────────────────────
const STATS = [
  { v: '5×', l: 'Faster site visit bookings' },
  { v: '85%', l: 'Lead qualification rate' },
  { v: '5–6h', l: 'Agent hours saved daily' },
  { v: '24/7', l: 'Always-on property AI' },
];

type Card = { icon: LucideIcon; t: string; d: string };

const CHALLENGES: Card[] = [
  { icon: MessageCircle, t: 'Ad leads going cold in Meta DMs', d: 'Click-to-WhatsApp ad leads wait hours for callback and buy elsewhere.' },
  { icon: CalendarX, t: 'Site visit no-shows', d: 'Prospective buyers book site visits but forget without automated reminders.' },
  { icon: ClipboardList, t: 'Manual floor plan sharing', d: 'Agents manually email brochures, floor plans and pricing PDFs to hundreds of leads.' },
];

const CAPABILITIES: Card[] = [
  { icon: CalendarCheck, t: 'Site visit scheduling', d: 'Buyers schedule and confirm site visits directly inside WhatsApp with live calendar sync.' },
  { icon: FileText, t: 'Instant brochure & floor plans', d: 'Deliver project brochures, floor plans, and layout PDFs in one click.' },
  { icon: BellRing, t: 'Automated visit reminders', d: 'Send GPS directions, agent details, and reminder nudges before the site visit.' },
  { icon: Sparkles, t: 'AI property assistant', d: 'Qualifies buyer budget, location preference, and configuration 24/7 before routing to agents.' },
  { icon: CreditCard, t: 'Booking deposit checkout', d: 'Share token payment links for unit blockings and instant digital receipts.' },
  { icon: HeartPulse, t: 'Project construction updates', d: 'Broadcast milestone progress photos and video updates automatically to buyers.' },
];

const HIGHLIGHTS = [
  {
    tag: 'Lead Qualification & Site Visits',
    title: 'Qualify property buyers and book site visits automatically',
    body: 'Filter serious buyers by budget, BHK configuration and timeline in real-time. Schedule site visits with automated calendar invitations and location pins.',
    points: ['Instant budget & requirement qualification', 'Automated site visit scheduling with GPS pins', 'Agent instant notification on high-intent leads'],
  },
  {
    tag: 'Post-Visit & Deal Closure',
    title: 'Nurture leads from first visit to token payment',
    body: 'Send automated follow-up messages after site visits, share price sheets, and collect token booking deposits right inside WhatsApp.',
    points: ['Automated post-visit feedback & follow-ups', 'Token payment links with instant receipts', 'Construction milestone updates for existing buyers'],
  },
];

const TESTIMONIALS = [
  { q: 'Click-to-WhatsApp ads integrated with WeNext doubled our site visit conversion rate within 30 days.', c: 'Prestige Realty', r: 'Real Estate Developer' },
  { q: 'Our sales agents spend zero time answering repetitive budget and location questions — AI qualifies every lead first.', c: 'Sobha Developers', r: 'Residential Projects' },
  { q: 'Automated site visit location pins and reminders cut our weekend visit drop-off rate by 50%.', c: 'Urban Spaces', r: 'Commercial & Housing' },
];

const FAQS = [
  { q: 'Can WeNext integrate with Meta Click-to-WhatsApp ads?', a: 'Yes. WeNext seamlessly connects Meta Click-to-WhatsApp and Instagram ads so incoming leads trigger instant AI lead qualification.' },
  { q: 'How does site visit booking work for prospective buyers?', a: 'Buyers select their preferred project location, date, and time slot right inside WhatsApp. They receive an instant calendar confirmation and Google Maps pin.' },
  { q: 'Can we send project brochures and floor plans automatically?', a: 'Yes. You can attach project brochures, floor plans, pricing sheets, and video walk-through links for immediate delivery in the chat.' },
  { q: 'Does it integrate with real estate CRMs like Salesforce or Zoho?', a: 'Yes. WeNext syncs lead status, visit dates, and conversation logs back into your CRM in real time.' },
  { q: 'Can sales agents take over when a lead is qualified?', a: 'Yes. Once the AI qualifies a buyer’s budget and interest, the lead is immediately assigned to a sales manager with a full summary.' },
];

const BENEFITS: { icon: LucideIcon; t: string; c: string; d: string; items: string[] }[] = [
  { icon: Users, t: 'Sales & Field Agents', c: '#0a8f5a', d: 'Close properties faster with qualified leads.', items: ['Instant lead routing with budget tags', 'Automated site visit calendar reminders', 'Instant brochure and price sheet dispatch'] },
  { icon: Stethoscope, t: 'CRM & Admin Team', c: '#3f6cab', d: 'Zero manual data entry between WhatsApp & CRM.', items: ['Auto-sync conversation notes to CRM', 'Track site visit attendance live', 'Manage buyer documents & KYC digitally'] },
  { icon: Megaphone, t: 'Performance Marketing', c: '#7c5bd6', d: 'Maximize ROI on Meta & Google ad campaigns.', items: ['Connect Click-to-WhatsApp ad campaigns', 'Track ad source per lead in chat', 'Retarget unbooked leads with offer broadcasts'] },
  { icon: Receipt, t: 'Finance & Booking Desk', c: '#c98a3a', d: 'Fast token payment collection inside WhatsApp.', items: ['Send token booking payment links', 'Generate instant booking receipts', 'Share payment schedule reminders'] },
];
    ...
    <div className="bg-[#f8f5ec] relative size-full min-h-screen flex flex-col overflow-x-clip">
      ...
      <div className="bg-[#092511] shadow-[0_0_0_100vmax_#092511] [clip-path:inset(0_-100vmax)] shrink-0 w-full">
        <div className="container mx-auto border-x border-[rgba(255,255,255,0.08)] px-4 xl:px-[75px] py-[90px] lg:py-[100px] flex flex-col items-center text-center relative overflow-hidden">
          ...
          <Eyebrow label="Real Estate Solution" light center />
          ...
          <Typography component="h1" className="font-['Geist:SemiBold'] font-semibold leading-[1.14] text-white text-[44px] md:text-[60px] tracking-[-2px] max-w-[840px] mb-[20px] relative">
            WhatsApp Site Visits,<br />Lead Qualification & <span className="text-[#25d366]">Property Sales</span>
          </Typography>

          <Typography className="font-['Geist:Regular'] text-[#a0b8a8] text-[18px] md:text-[20px] max-w-[660px] leading-[1.6] mb-[40px] relative">
            Connect ad leads to instant site visits — automated qualification, instant brochure sharing, token payment links and 24/7 AI lead nurturing.
          </Typography>

          <div className="flex flex-wrap gap-[16px] items-center justify-center relative">
            <PrimaryButton label="Book a Real Estate Demo" />
      ...
      <div className="container mx-auto border-x border-b border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[36px] flex flex-col items-center gap-[20px]">
          <Typography className="font-['Courier_Prime'] text-[#8a938e] text-[12.5px] uppercase tracking-[0.18em]">Trusted by top builders & developers</Typography>
          <div className="flex flex-wrap items-center justify-center gap-x-[44px] gap-y-[18px]">
            {['Prestige Realty', 'Sobha Developers', 'Urban Spaces', 'Skyline Infra', 'Apex Homes'].map((n) => (
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            What’s quietly dropping your site visit conversions
          </Typography>
          <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] max-w-[560px] leading-[1.55] mt-[14px]">
            Three everyday sales friction points cost real estate teams site visits and bookings — WeNext resolves all three.
          </Typography>
        </div>
      </div>
      ...
      <div className="container mx-auto border-x border-[#e0dac6]">
        <div className="px-4 xl:px-[56px] py-[56px] pb-[36px]">
          ...
          <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[38px] text-[#0c221f] tracking-[-1.2px] leading-[1.14] max-w-[600px]">
            Built specifically for real estate developers & brokers
          </Typography>
        </div>
      </div>
      <div className="container mx-auto">
        ...
        <div className="px-4 xl:px-[56px] py-[64px]">
          <div className="max-w-[600px] mb-[40px]">
            ...
            <Typography component="h2" className="font-['Geist:SemiBold'] font-semibold text-[42px] text-[#0c221f] tracking-[-1.4px] leading-[1.14]">
              How your whole sales engine benefits
            </Typography>
            <Typography className="font-['Geist:Regular'] text-[#60584c] text-[17px] leading-[1.55] mt-[14px]">
              One shared WhatsApp workspace — sales, CRM, marketing and finance working together in sync.
            </Typography>
          </div>
              ...
              <div className="content-stretch flex flex-col gap-[30px] items-start relative shrink-0 w-full lg:w-[380px]">
                <div className="content-stretch flex flex-col gap-[15px] items-start relative shrink-0">
                  ...
                  <div className="font-['Geist:SemiBold'] font-semibold relative shrink-0 text-[#0c221f] text-[36px] lg:text-[42px] tracking-[-1px]">
                    <Typography className="leading-[1.3] text-[#0c221f] text-[36px] lg:text-[42px] font-['Geist:SemiBold'] font-semibold">Questions developers ask</Typography>
                  </div>
                </div>
              </div>
